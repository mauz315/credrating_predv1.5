...
def order_ratings(labels):
    base = ["AAA", "AA+", "AA", "AA-", "A+", "A", "A-", "BBB+", "BBB", "BBB-",
            "BB+", "BB", "BB-", "B+", "B", "B-", "CCC+", "CCC", "CCC-", "CC", "SD"]
#    C = ["CCC+", "CCC", "CCC-", "CC", "C", "SD"]
#    d = []
#    for i in labels:
#        if i in C:
#            d.append("CCC")
#        else:
#            d.append(i)
    return [i for i in base if i in labels]
...
#    plt.show()
    ...
    # Data Permutation: Por si es necesario volver 
    X_o = X
    y_o = y
    if permut:
...
def rating_prediction(data, rf, feat_key, le, sov_lab_encoder, output_file):
...
#    pred_calif_pure = np.array([le.iloc[x == list(le.iloc[:,0]),0].index[0] for x in rf_pure.predict(X_new_pure)])
    ...
    rat_dist = np.array([int(le.loc[x]) for x in sob_rating]) - np.array([int(le.loc[x]) for x in pred_calif])#pred_calif_pure
